...
def ask_gemini(question, projects, netsuite_data=None, scope="brendan"):
    ...
    prompt = (
        ...
        "- 'WAITING ART' = items awaiting artwork design (not yet paid)\n"
        "- 'PAID/WAITING ART' = items paid and awaiting artwork design\n"
        "- 'QUOTE NEEDED' = items that need a price quote only, NOT artwork related\n"
        "- 'QUOTE ADDED' = quote has been provided, awaiting next step\n"
        "- 'ARTWORK CONFIRMED' = artwork approved, moving to production\n"
        "- 'PART CONFIRMED' = partially confirmed\n"
        "- 'PLATING' or 'POLISHING' = in production finishing stages\n"
        "- 'PART SHIPPED' = partially shipped\n"
        "- 'SHIPPED' or 'RESOLVED/SHIPPED' = completed and shipped\n"
        "- 'NEEDS RESOLUTION' = there is an issue with this order\n"
        "IMPORTANT: When asked about items 'awaiting artwork' or 'needs artwork', ONLY return records where Status is 'WAITING ART' or 'PAID/WAITING ART'. Do NOT include 'QUOTE NEEDED' items - those need quotes not artwork.\n"
        "\n"
        "--- LARK PROJECT DATA ---\n" + context + "\n--- END LARK DATA ---\n"
        + netsuite_section +
        "\nQuestion: " + question + "\nAnswer:"
